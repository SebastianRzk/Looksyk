...
mod tests {
    ...
    #[test]
    fn test_would_run_migration_with_migration_pending_should_return_true() {
        let current_version = ApplicationVersion::new("1.10.2");
        let data_state_version = ApplicationVersion::new("1.10.0");

        let result = would_run_migrations(&current_version, &data_state_version);

        assert!(result)
    }

    #[test]
    fn test_would_run_migration_with_no_migration_pending_should_return_false() {
        let current_version = ApplicationVersion::new("1.10.2");
        let data_state_version = ApplicationVersion::new("1.10.2");
    ...
    #[test]
    #[should_panic(expected = "Data state version 1.10.2 is newer than current version 1.10.0")]
    fn test_would_run_migration_with_illegal_datastate_should_abort() {
        let current_version = ApplicationVersion::new("1.10.0");
        let data_state_version = ApplicationVersion::new("1.10.2");

        would_run_migrations(&current_version, &data_state_version);
    }
}
