...
pub fn render_as_svg(plot: &PlotData, design: &Design) -> Result<String, Error> {
    let (min_y, max_y) = plot
        ...
        .points
        .iter()
        .fold((i32::MAX, i32::MIN), |(min, max), p| {
    ...
    let axis_color = css_to_plotters_rgba(
        &design.foreground_color,
        match design.appearance {
            crate::looksyk::data::config::runtime_graph_configuration::Appearance::Light => {
                BLACK.mix(0.8)
            }
            crate::looksyk::data::config::runtime_graph_configuration::Appearance::Dark => {
                WHITE.mix(0.8)
            }
        },
    );
    let grid_color = css_to_plotters_rgba(
    ...
    let label_color = css_to_plotters_rgba(
        &design.foreground_color,
        match design.appearance {
            crate::looksyk::data::config::runtime_graph_configuration::Appearance::Light => {
                BLACK.to_rgba()
    ...
    {
        let root =
            SVGBackend::with_string(&mut svg_buf, (plot.width, plot.height)).into_drawing_area();
        ...
        let mut chart = ChartBuilder::on(&root)
            ...
            .build_cartesian_2d(
                plot.data.points[0].date..plot.data.points.last().unwrap().date,
