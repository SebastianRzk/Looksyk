...
pub fn generate_asset_preview(
    ...
    match media_type {
        MediaType::Image => AssetPreview {
            markdown_preview: Some(render_asset_as_image(&asset_descriptor)),
        ...
        MediaType::Code => AssetPreview {
            markdown_preview: Some(
                render_code_block(
                    &MediaOnDisk::new(&asset_descriptor),
                    cache,
                    data_root_location,
                )
                .inplace_markdown,
...
mod tests {
    ...
    fn test_generate_asset_preview_should_return_image_preview() {
        ...
        let result =
            generate_asset_preview(asset_descriptor, file_size, &mut cache, &data_root_location);

        assert_eq!(
            result.markdown_preview.unwrap(),
            "![filename.png](/assets/filename.png)"
        );
        assert_eq!(result.html_preview_link, None);
        assert_eq!(result.properties.size, "100 B");
        assert_eq!(
            result.properties.full_qualified_path,
            "/assets/filename.png"
        );
    }
