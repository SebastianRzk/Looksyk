...
pub fn update_index_for_file(file_id: PageId,
    ...
    let mut new_page_associated_state = remove_page_from_internal_state(&file_id, &page_type, &simple_page_name, page_associated_state);
    ...
    match page_type {
        PageType::UserPage => {
            new_page_associated_state.user_pages.entries.insert(simple_page_name.clone(), update.clone());
        }
        PageType::JournalPage => {
            new_page_associated_state.journal_pages.entries.insert(simple_page_name.clone(), update.clone());
        }
    }
...
pub fn remove_page_from_internal_state(file_id: &PageId, page_type: &PageType, simple_page_name: &SimplePageName, page_associated_state: CurrentPageAssociatedState) -> NewPageAssociatedState {
    let new_page_index;
    let new_journal_index;
    if let PageType::UserPage = page_type {
        new_page_index = remove_file_from_page_index(&page_associated_state.user_pages, &simple_page_name);
        new_journal_index = page_associated_state.journal_pages.clone();
    } else {
        new_journal_index = remove_file_from_journal_index(&page_associated_state.journal_pages, &simple_page_name);
    ...
    let new_todo_index = remove_file_from_todo_index(&page_associated_state.todo_index, &simple_page_name);
